...
const fetchNotifications = async () => {
  ...
  function CreatePost() {
    ...
    const handleChange = (event) => {
        ...
        // Cập nhật trạng thái cho các trường trong phone
        if (name in newPost.phone) {
            setNewPost((prevPost) => ({
                ...prevPost,
                phone: {
                    ...prevPost.phone,
    ...
    return (
        ...
        <Box component="form" onSubmit={handleSubmit} sx={{ flexGrow: 1, padding: 2 }}>
            ...
            {[
                { label: 'Thương hiệu', name: 'brand', required: true },
                { label: 'Tình trạng', name: 'condition', required: true },
                { label: 'Mẫu', name: 'model', required: true },
                { label: 'Nguồn gốc', name: 'origin', required: true },
                { label: 'Màu sắc', name: 'color', required: true },
                { label: 'Dung lượng', name: 'storage', required: true },
                { label: 'Giá', name: 'price', required: true },
                ...
                <FormControl variant="outlined" fullWidth sx={{ marginBottom: 2 }} key={index}>
                    ...
                    <TextField
                        ...
                        placeholder=" "
                        size="large"
                        value={newPost[field.name]} // Cập nhật giá trị từ newPost
                        name={field.name}

    ...
    function CreatePost() {
        ...
        const handleSubmit = async (event) => {
            ...
            try {
                ...
                const { message, data} = response.data; // Giả sử bạn nhận được postId từ phản hồi
    
                if (message === 'Success') {
                    // Nếu tạo bài viết thành công, tải ảnh lên
                    if (image) {
                        ...
                        try {
                            ...
                            if (uploadResponse.data.message === 'Upload Successfully!') {
                                ...
                                alert('Tải ảnh lên thất bại: 1' + uploadResponse.data.message);
                            }
                        } catch (error) {
                            console.error('Error uploading image:', error);
                            alert('Tải ảnh lên thất bại 2');
                        }
                    }
        ...
        return (
            <>
                <Box sx={{ padding: 2 }}>
                    <Typography variant='h6' style={{ fontWeight: 'bold' }} align='left' color='#000000'> Hình ảnh sản phẩm</Typography>
                ...
                <Box component="form" onSubmit={handleSubmit} sx={{ flexGrow: 1, padding: 2 }}>
                    <Typography variant='h6' style={{ fontWeight: 'bold', marginBottom: 10 }} align='left' color='#000000'>
                        Thông tin sản phẩm và tiêu đề tin đăng
                    </Typography>
                    {[
                        { label: 'Hãng xe', name: 'manufacturer', required: true, vehicle: true },
                        { label: 'Xuất xứ', name: 'origin', required: true, vehicle: true },
                        { label: 'Năm sản xuất', name: 'year', required: true, vehicle: true },
                        { label: 'Nhiên liệu', name: 'fuelType', required: true, vehicle: true },
                        { label: 'Màu sắc', name: 'color', required: true, vehicle: true },
                        { label: 'Biển số', name: 'licensePlate', required: true, vehicle: true },
                        { label: 'Giá cả', name: 'price', required: true },
                        { label: 'Số km đã đi', name: 'mileage', required: true, vehicle: true },
                        ...
                        <FormControl variant="outlined" fullWidth sx={{ marginBottom: 2 }} key={index}>
                            ...
                            <TextField
                                ...
                                placeholder=" "
                                size="large"
                                value={field.vehicle ? newPost.vehicle[field.name] : newPost[field.name]}
                                name={field.name}
